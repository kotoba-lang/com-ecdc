(ns ecdc.main
  "Kotodama WASM entrypoint for the ECDC clean-room actor (L4) — Clojure port.
  ...
  facts (`ecdc.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"ecdc.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ecdc-compat" :tier tier :entities entities} 200])
